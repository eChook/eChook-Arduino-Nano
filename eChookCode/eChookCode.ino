...
/** BUILD OPTIONS                      */
/** ================================== */
const int DEBUG_MODE = 1; //if debug mode is on, no data will be sent via bluetooth. This is to make any debug messages easier to see.
...
/* THROTTLE */
long nextThrottleReadMs = 0; // This has to be long as an int can't hold a large enough number.
int currThrtlOut = 0;
...
int readThrottle() //This function is for a variable Throttle input. In the calculations maximum throttle is taken as 255 as that is the max 8-bit PWM output.
{
        ...
        Serial.println(currThrtlOut);

        analogWrite(MOTOR_OUT_PIN, currThrtlOut); //This drives the motor output. Unless you are using the board to drive your motor, comment it out.

        return (currThrtlOut); // the (int) converts the value into an integer value before the return function uses it
}
