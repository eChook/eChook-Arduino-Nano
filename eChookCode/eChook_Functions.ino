...
void eChookRoutinesUpdate()
{
        ...
        if (millis() > nextThrottleReadMs)
        {                                            // millis() gives milliseconds since power on. If this is greater than the nextThrottleReadMs we've calculated it will run.
...
int readThrottle()
{
        ...
        if (CAL_THROTTLE_RAMP)
        {
                ...
                if (tempThrottle >= currThrtlOut && tempThrottle > 100)
                { // This could be if(thrtlIn > thrtlOut && speed < threshold) to make it low speed only. Speed and threshold are undefined in this example!
                        if (currThrtlOut < 100)
                        {
                                currThrtlOut = 100;
                        }
                        ...
                        if (currThrtlOut > tempThrottle)
                        { // Fixes the throttle jitter if the increment puts output over request.
...
float readWheelSpeed()
{
        ...
        if (CAL_USE_IMPROVED_SPEED_CALCULATION)
        { // Two ways to calculate wheel RPM
                // Calculate time taken for last wheel rotation
                float wheelRPS = 0;
                if (millis() - lastWheelPollTime < 2000)
                { // if over 2 seconds since a poll is seen, assume stopped.
                        if (lastWheelInterval < 2000)
                        {                                                                    // catches the case of first pulse in a while
...
float readMotorRPM()
{
        if (CAL_USE_IMPROVED_RPM_CALCULATION)
        {
                float tempRpm = 0;
                // Calculate time taken for last wheel rotation
                if (micros() - lastMotorPollTime < 1000000)
                { // If it takes longer than 2s for a rotation, consider it stopped.
                        if (lastMotorInterval < 1000000)
                        {                                                                             // catches the case of first pulse in a while
